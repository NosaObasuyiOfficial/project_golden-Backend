...
export const electricity_bills_processing = async(req: Request, res: Response) => {
    try{
          /*-------------------------Validating Input -------------------------------*/
            ...
            if (token) {
                ...
                if (getting_customer_data?.dataValues) {
                    ...
                    if(getting_customer_data.dataValues.account_type === 'onabill_admin_101'){
                        ...
                        const getting_specific_transactions = await Electricity_Payment.findOne({
                            where:{
                                tnx_id:  valid_tnx_id
                            }
                        })
                          ...
                          if(getting_specific_transactions.dataValues.status !== 'success'){
                            ...
                            if(getting_user_phoneNumber){
                                ...
                                if(valid_status === "success"){
                                    ...
                                    const withdrawal_amount = getting_specific_transactions.dataValues.amount   
                                
                                    await Electricity_Payment.update({status:valid_status},{
                                        where: {
                                          [Op.and]: [
                                            { tnx_id: valid_tnx_id },
                                      ...
                                      const withdrawal_balance = find_user_wallet_balance - withdrawal_amount

                                       await Client_Wallet.update({wallet_balance: withdrawal_balance},{
                                        where: {
                                          phoneNumber: find_user_phoneNumber
                                        },
                                      });
                              ...
                              if (
                                ...
                                const updating_user_transactions: any = await Electricity_Payment.update({status:valid_status},{
                                    where: {
                                      [Op.and]: [
                                        { tnx_id: valid_tnx_id },
                                        { user_id: find_user_id },
        ...
        console.error("Error processing electricity bill payment", error);
