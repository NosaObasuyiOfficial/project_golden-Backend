...
if( valid_phoneNumber !== valid_retype_phoneNumber || valid_password !== valid_retype_password){
  ...
  const checking_user_existence = await Onabill_Admin_Signup.findOne({
    where:{
  ...
  if(!checking_user_existence){
    ...
    const new_user = await Onabill_Admin_Signup.create({
      id:v4(),
      ...
      account_type: "onabill_admin_101",
      referral_id: "",
      service_status: true,
      block: false    
    })

    const new_user_created = await Onabill_Admin_Signup.findOne({
      where:{
        phoneNumber:valid_phoneNumber
      }
    })
    ...
    return res.status(400).json({
      message: `${valid_phoneNumber} is already an admin.`
    })
  }
}
